...
object DeviceAccountBind_v1 {
	def main(args: Array[String]): Unit = {
		...
		val curDayLog: DataFrame = spark.read.table("ods.ods_app_event_log")
			.where(" dt='2021-02-02' and eventid is not null")
			.selectExpr("deviceid", "if(account='', 'null', account) account",
				"sessionid", "timestamp")

//		curDayLog.show(20)
		curDayLog.createTempView("curDayLog")
		//根据deviceid, account，sessionid，timestamp聚合当天的日志，得出每一种“设备-账号”的会话数，并转换为score
...
//		curDayCombineSessionCnt.show(10)
		...
		val curBindScoreResult: DataFrame = spark.sql(
			"""
				|insert into table dws.dws_device_account_bind_score partition(dt='2021-02-02')
				|select
