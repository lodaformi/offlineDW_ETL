...
object OdsApp2DwdApp_v2 {
	...
	def main(args: Array[String]): Unit = {
		...
		val curDate = "2021-02-06"
		var pre_date = DateUtil.getLastDate(curDate)
		var cur_date = DateUtil.getCurrentDate(curDate)
	...
	def dataSave(spark: SparkSession, res: Dataset[LogBean], cur_date: String): Unit = {
		...
		spark.sql(
			s"""
				 | insert overwrite table dwd.dwd_app_event_detail partition(dt='${cur_date}')
				 | select
				 | account   ,
				 | appId       ,
				 | appVersion  ,
